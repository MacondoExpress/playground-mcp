...
const writeCypherTool = server.registerTool(
  "write-cypher",
  {
    ...
    inputSchema: {
      cypherQuery: z.string().default('CREATE(n:User) SET n.name = "MPC-USER"'),
      url: z.string().url().default("bolt://localhost:7687"),
    },
  },
  async ({ cypherQuery, url }) => {
...
async function executeQuery(cypher: string, readOnly = true): Promise<string> {
...
async function main() {
  // TODO: STDIO implement only, implement HTTP/SSE
  ...
  console.error("Cypher MCP Server running on stdio");
}
